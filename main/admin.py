...
from .models import UserProfile, Site, Operation, Item, AppSettings
...
class AppSettingsAdmin(admin.ModelAdmin):
    """Ограничиваем админку на singleton-настройки."""

    list_display = ('local_site_name', 'office_email', 'updated_at')

    def has_add_permission(self, request):
        # Не даём плодить записи
        return False
    ...
    def changelist_view(self, request, extra_context=None):
        # Если кто-то полез в список, сразу перекидываем на единственную запись
        obj = AppSettings.get_solo()
        from django.shortcuts import redirect
        return redirect(f'../appsettings/{obj.id}/change/')