...
def home(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Генерим накладную и отправляем письмо по всем операциям, кроме прихода.
            if op_type != OperationType.INCOMING:
    ...
    ).select_related('from_site', 'to_site', 'created_by', 'item').order_by('-created_at')[:5]
...
def stocks(request):
    ...
    # Добавление ТМЦ перенесено сюда (в остатки).
    if request.method == 'POST':
        if request.POST.get('form_type') == 'add_item':
            ...
            inv = (request.POST.get('item_new_inv') or '').strip() or None
            ...
            item, created = Item.objects.get_or_create(
                name=name,
                defaults={'default_unit': unit, 'sku': inv},
            )

            # Если ТМЦ уже было, можно обновить единицу/инвентарник по желанию.
            if not created:
                changed = False
                if unit and item.default_unit != unit:
                    item.default_unit = unit
                    changed = True
                if inv and item.sku != inv:
                    item.sku = inv
                    changed = True
                if changed:
                    item.save()
            ...
            if created:
                ...
                messages.info(request, f'ТМЦ "{item.name}" обновлено/уже существует')
            return redirect('/stocks/')
...
def operations_list(request):
    """Список операций (полная лента)."""
    local_site = get_local_site()
    local_filter = Q(from_site=local_site) | Q(to_site=local_site)

    # Простые фильтры (без усложнений): поиск по ТМЦ/комменту/получателю/транспорту.
    q = (request.GET.get('q') or '').strip()
    qs = Operation.objects.filter(local_filter).select_related('from_site', 'to_site', 'created_by', 'item').order_by('-created_at')
    if q:
        qs = qs.filter(
    ...
    # Чтобы не убить браузер, ограничим разумным числом.
    operations = list(qs[:500])

    return render(
        request,
        'main/operations.html',
        {
            'local_site': local_site,
...
def operation_invoice_generate(request, pk: int):
    ...
    if op.operation_type == OperationType.INCOMING:
        messages.info(request, 'Для прихода накладная не формируется.')
        return redirect('/')
